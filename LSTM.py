...
#to process tuples
def collate_fn(data):
    text_batch, labels = zip(*data)
    return text_batch, labels
...
def train_cycles(X_all, y_all, vocab, num_samples, model_type, epochs, patience, batch_size, seq_len, lr, clip, log_level):
    result = pd.DataFrame(columns=['Accuracy', 'F1(macro)', 'Precision', 'Recall', 'Total_Time', 'ms/text'], index=num_samples, dtype = object)
    ...
    #changed from 5 to 2 for lstm and bert
        ...
        acc, f1, precision, recall, model_trained = train_nn_model(model, model_type, train_loader, valid_loader, vocab, epochs, patience, batch_size, seq_len, lr, clip, log_level)
        end_time = time.perf_counter() # use time.process_time() for CPU time
        duration = end_time - start_time
        logging.info("Process Time (sec): {}".format(duration))
        result.loc[n] = (round(acc,4), round(f1,4), round(precision,4), round(recall,4), duration, duration/n*1000)
...
def train_nn_model(model, model_type, train_loader, valid_loader, vocab, epochs, patience, batch_size, seq_len, lr, clip, log_level):
   ...
   # logger = set_logger('sa_tweet_inperf', log_level)
    ...
    for epoch in tqdm(range(epochs), desc="Epoch"):
        ...
        logging.info('Epoch: %d, Loss: %.4f, Acc: %.4f, F1: %.4f, LR: %.2e, P: %.4f, R: %.4f' % (epoch+1, loss_valid[-1], acc_valid[-1], f1_valid[-1], scheduler.get_last_lr()[0], precision_valid[-1], recall_valid[-1]))
...
Train_X, Test_X, Train_Y, Test_Y = model_selection.train_test_split(pd.DataFrame({'story':preprocessed}),df_labels['label'],test_size=0.2)
...
#print(len(Test_X))
test_loader = create_data_loader(Test_X, Test_Y, list(range(0,len(Test_X))), len(Test_X), False)
...
# Run LSTM
result_lstm, model_trained_lstm = train_cycles(Train_X_over, Train_Y_over, vocab, num_samples, 'LSTM', epochs, patience, batch_size, seq_len, lr, clip, log_level)

# Save the model and show the result
path = "./campaign_lstm.pth" 
torch.save(model_trained_lstm.state_dict(), path) 
...
#             #print(type(inputs))
#             inputs = tokenizer_lstm(inputs, vocab, seq_len=30, padding='left').transpose(1,0).to(device)
#             hidden = model.init_hidden(64)
#             hidden = tuple([each.data for each in hidden])
#             predicted_outputs = model(inputs, hidden) 
